...
In spite of an innovative idea, there are few publications
...
In 2017, a breakthrough is came out. We develop an algorithm called
...
technique, we can render image of a quasi-sphere in real-time.
For more details of IIS, see
\cite{bridges2016} or \cite{bridges2017}.

Our previous paper \cite{bridges2018}, is focused on the artistical
...
Also, an infinite type of sphairahedron can be tiled, as presented in Figure
...
We call $\varepsilon(n_{ij})$ moduli of polyhedra.
...
\begin{lemma}\label{eightVertices}
 Eight vertices of a polyhedron of cube-type lie in the same sphere.
\end{lemma}
...
\begin{proof}
 ...
 three vertices $A, D, G,$ and $F$ lie on one planar circle, all vertices
...
$G'(x_G, y_G)$ be projection of these seven vertices to the xy-plane.
...
\begin{align*}
 O_2(1 - r_2, 0, 0).
\end{align*}
From Lemma\ref{eightVertices}, 7 points A, B, C, D, E, F, G lie on one plane. D is on
the segment $AB$ and
\begin{align*}
...
\begin{proof}
 In order to satisfy ideality, a sum of the edge angles of each vertex
...
\begin{proof}
 ...
 $B'_{x'} \cap P'$ is right cylinder infinitely elongated for negative
