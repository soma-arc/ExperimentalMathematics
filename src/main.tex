...
In 2016, Ryo Kageyama decided the parameter space of the cube type
...
One of the example is Figure
\ref{fig:sphairahedron}\subref{fig:sphairaPrismHalf}.
Here, we consider the shape infinitely long triangular prism hollowed
out by three balls, and one of the connected components is sphairahedron.

In infinite, sphairahedron 
a face adjacent to infinite-point is the boundary of half-space, that is to
say, a plane.
In the neighborhood of infinite-point, sphairahedron becomes prism.
When the closure of sphairahedron doesn't include infinite-point,
...
Moreover, we can loosen the definition of sphairahedron, that is, case
$A$ has simply connected three or more components.
\begin{definition}
...
two components) a semi-sphairahedron.
\end{definition}
Figure \ref{fig:semi-sphairahedron}\subref{fig:semi-sphairaAll} shows an
example of a semi-sphairahedron.  We can get this shape by scooping a
triangle prism out by two balls.  Complement $A$ consists of three
connected components. Lower two of them are tetrahedra, and the upper one is a
pentahedron.  We can regard the unions of lower two components as a
sphairahedron with a singular point.

See Figure \ref{fig:semi-sphairahedron}\subref{fig:semi-sphairaHalf}.
We can regard the resulting shape as a singular case of a pentahedron,
and we call it a \textit{semi-sphairahedron}.  Remark that Ahara, Araki,
and Kageyama did not deal with a semi-sphairahedron and did not render
its tiling patterns.

Other than semi-sphairahedra, we have much more 'sphairahedron-like
shapes' in this situation.  See Figure \ref{fig:brokenHole}. In this
