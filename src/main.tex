...
There are many pictures of three-dimensional, so it is hard to see whole
...
First of all, we define \textit{Limit ball}.
\begin{definition}
 Limit ball is the shape whose limit set is its boundary surface
 and we obtain it by tiling of the sphairahedron $P$.
$$
 G = G_p := \left< \text{inversion} of S | S \text{is surface of} P \right>,
$$
Limit ball of $P$ : LimitBall(P) := $G_p(P)$
\end{definition}
...
$G'(x_G, y_G)$ be projection of these 7 vertices to the xy-plane.
...
In this section, we show other types of the sphairahedra based on the
...
Tetrahedron has the fewest number of the faces.
...
The tiling pattern of planes are different from infinite sphairahedra.
...
pentahedral pyramid don't have parameter space. It has two patterns
...
When parameter is positive, it seems like plane as shown in 
...
Hexahedral cake is shown in Figure \ref{fig:cake}. There are three
types of sphairahedra based on the cake.
Infinite sphairahedra generate landscape-like pattern.
They have organic shape.
The sphairahedron has three type of the combinations of face angles.
...
Firstly, the sphairahedra have many combinations of arrangement of
vertexes and edges. 
Heptahedron has thirty four types of arrangement and
the number of the polyhedra with eight or more faces explosively
increases. See also \cite{countingPolyhedra}.
Secondly, to satisfy rationality of the sphairahedra, they should not
...
\begin{proof}
 In order to satisfy ideality, sum of the edge angles of the each vertex
 should be $(k - 2) \pi$ where $k$ is the number of the edges connecting
 the vertex and $k \geq 3$ for natural number $k$.

 For rationality, 
 largest edge angle is $\pi / 2$.
 Sum of the angle is $k\pi / 2$
 $(k -  2)\pi > k\pi / 2$
 ...
 To satisfy ideality, sum of the edge angles should be $(k - 2) \pi$
 we transform the vertex to infinity.
 the largest face angle is $\pi / n (n > 1)$
 $(k - 2)\pi$
\end{proof}

So, the condition of vertexes are greatly restricted.
...
After we compute sphairahedra, we have to draw infinite number of
...
We successfully implement fast rendering algorithm
...
Tile sphairahedra directory using elements of the group.
...
quasi-sphere same to the tiling pattern of the sphairahedron.
...
\begin{proposition}
 In order to obtain tiling patterns of sphairahedra, we apply a group to
 sphairahedra. 
 We consider sum of spheres include a sphairahedron, and we transform
 the spheres using the group. and we obtain quasi-sphere.
\end{proposition}
...
\subsection{All of the parameter is zero}

Cube type sphairahedron type (a) is controlled by two parameter.
When the both parameters are zero, the limit set becomes plane (or
sphere) and like Figure there is regular patterns. We will analyze the
patterns.

When $z_b = z_c = 0$, the every center of the three sphere $C_A, C_B, C_C$
are on the x-y-plane. So about the face of the triangle prism and all of the
$C_A, C_B, C_C$, we consider crossing with x-y plane, we obtain Figure
\ref{}.
By we apply proper similarity transformation, we can make $P$ and $Q$ in
Figure {} to
$Q_3 = { a+bw | w=\dfrac{1 + \sqrt{3}i}{\alpha}, a, b \in \mathbb{Z}}$
(Eisenstein ring)

At this time about the line symmetry transformation related to edge of
...
Similar phenomenon is happen in hexahedral cake type sphairahedron.
In this time, the patterns are originated from M\"obius transformations
on $\mathbb{Z}[i]$ gaussian ring of integer.

Other types will have some relation about finite index field on
 $\mathbb{Q}$ of ring of integers.

\subsection{Shape of the parameter pace}
...
\subsection{Ghost Sphere}
The shape of the limit set makes circle.
We call it ghost circle. In the three-dimensional space, there are ghost
...
\begin{definition}
 Let P be an ideal sphairahedron and let X be one of the vertex at P.
 Remark that, The edges are mutually tangent at the vertex X.
 Open sphere $B_x$ is maximum vertex when the sphere meet as below

 \begin{description}
  \item[1)] The boundary surface $\eth B_x$ of sphere $B_x$ pass through
             the vertex $X$ and $\eth B_x$ is orthogonal to all of the
             edges gathered at vertex $X$.
  \item[2)] $B_X \cap P \neq \phi$ and $B_X$ don't contains a vertex of
             $P$
  \item[3)] $B_X$ is a sphere satisfies 1) and 2)
             maximal to inclusive relation.
 \end{description}

Remark: \\
1)maximum vertex sphere is well-defined to each vertex of sphairahedron
$P$.
2) When the vertex $X$ of the sphairahedron $P$ is infinity point,
 crossing with $P$ 
 maximum half space doesn't contain the vertex of $P$ (not infinity
 point) is maximum vertex sphere of $X$.
\end{definition}

\begin{proposition}\label{prop:limitBall}
 For sphairahedron $P$ and its vertex $X$, limit ball defined by $P$
 includes maximum vertex sphere $B_x$ in $x$.
\end{proposition}

\begin{proof}
 Maximum vertex ball is equivariant for M\"obius transformations in
 ...
 transformation $F$, we can do following operations.
 \begin{description}
  \item[a)] transform $X$ to infinity point.
  \item[b)] $F(P)=P'$ neighborhood of $F(x)$ is infinitely elongated
             to z-axis negative direction right cylinder.
 \end{description}
 At this time, due to the above remark, in vertex $X' = F(X)$
 $F(Bx = B'x')$ is maximum open half space not include all of the vertex of
 $F(P)$ (other than $x'$).
 
 It is given by $B'x' = {(x, y, z) | z < z_0}$ when
 $z_0 := \min{ z | (x, y, z) \text{is an vertex of} F(P) }$

$V'_{x'} \cap P'$ is right cylinder infinitely elongated for z-axis
 negative direction and its dihedral angle is $pi/n$ where $n \geq 2$
 for $n$ is integer.
...
\begin{theorem}
 When $\bigcup_{x \text{is vertex of} P} B_x \subset P$,
 $\bigcup_{x \text{is vertex of} P} G_{B_x} = G_{p}(P)$,
\end{theorem}
...
\end{proof}
Remark that by this in order to render limit ball,
for each vertex $X$, we compute $B_x$ and render $G_p(\bigcup_x B_x)$
...
\begin{figure}[H]
 \begin{minipage}[t]{0.3\textwidth}
  \centering
  \includegraphics[width=1in, height=1in, keepaspectratio]{./img/visualization/cubePrism.png}
  \subcaption{Step 5}
  \label{}
 \end{minipage}
 \hspace*{\fill}
 \begin{minipage}[t]{0.3\textwidth}
  \centering
  \includegraphics[width=1in, height=1in, keepaspectratio]{./img/visualization/cubeSmallHole.png}
  \subcaption{Step 10}
  \label{}
  \end{minipage}
 \hspace*{\fill}
 \begin{minipage}[t]{0.3\textwidth}
  \centering
  \includegraphics[width=1in, height=1in, keepaspectratio]{./img/visualization/cubeHole.png}
  \subcaption{Final image}
  \label{}
 \end{minipage}
 \hspace*{\fill}
...
In this case, the sprhairahedron corresponding to the limit set may not
rational.
...
because the vertex have many edges, and gather tiles.
...
It is not discrete, but when the arms crossing at rational angle, it becomes
discrete.
...
\begin{figure}[h!tbp]
 ...
 \hspace*{\fill}
\end{figure}
When all of the parameter is zero, the limit set becomes plane.
The patterns of plane are different from sphairahedron respectively.
See Figure \ref{fig:planes}.
The blue points represent patterns of parabolic fixed points.
The patterns of plane is generated by two-dimensional circle inversion
fractals, so we prepare half planes and inversion circles.
It can be generated by Schottky Link (https://schottky.jp).


We can also find that the blue parts themselves form the constant
patterns. The point is parabolic fixed point and the convergence is
slow.
The vertex which many circles cross is slow convergence.
