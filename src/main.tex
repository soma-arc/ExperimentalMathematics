...
\begin{proposition}
Assume that the sphairahedron $T$ has the infinite vertex.  In any case of dihedral angles, the limit set is the union of a plane and $\infty$.
\end{proposition}
...
\]
See Figure \ref{fig:tetrahedronInf}.  If we regard the lower half plane $\{(x,y,z) \mid z<0\}$ as the hyperbolic space $\mathbb{H}^3$, $T$ is a hyperbolic ideal tetrahedron.  If the tiling $L=(T,G)$, then the boundary of the total tiling space $\varlimsup_{k\to\infty} |L|_k$
coincides to $\partial{\mathbb{H}^3}$ and the limit set is the $xy$-plane.
\end{proof}

Indeed, the quasi-spheres are shown in Figures \ref{fig:tetrahedronInf}, \ref{fig:tetrahedronInf_b},
and \ref{fig:tetrahedronInf_c}.  They are $xy$-plane.  They have different colored patterns and it is because the combinations of the dihedral angles are different from each other.  The group $G$ of $T$ is 3-Kleinian.  If a tetrahedron is finite, the limit set is a sphere as seen in Figure \ref{fig:tetrahedronFinite}.
...
$S^3\setminus(\overline{D_1}\cup\overline{D_2}\cup\overline{D_3}\cup\overline{D_4})$ is a rectangular column. The vertex set is $P_{125}, P_{235}, P_{345}, P_{145}, \infty$, where $P_{ijk}= O_i\cap O_j\cap O_k$.  Suppose that $P_{125}$ is on the $xy$-plane, that is, $P_{125}=(x_1, y_3, 0)$.  $z_5=0$ follows from Lemma \ref{lemma:CenterOfSphere}(1).  Lemma \ref{lemma:CenterOfSphere}(3) indicates that the center $(x_5, y_5, 0)$ is on the plane $\sqrt{3}(x-x_1)-(y-y_3)=0$.  See Figure \ref{fig:proofQudrangularPyramid}.  Because the center $(x_5, y_5,0)$ is on the $xy$-plane, $P_{345}$ must be on the same plane, that is, $P_{345}=(-x_1, -y_3, 0)$.  This implies that $(x_5, y_5, 0)$ also on the plane $\sqrt{3}(x+x_1)-(y+y_3)=0$ and hence we have $\sqrt{3}x_1 = y_3$ and $x_5=y_5=0$.

Thus the all coordinate are determined in one way except dilation and the parameter space consists of one point.  In case of the type (b) of dihedral angles, we can show it in the same way.  In this case, $x_1 = y_3$ and $(z_5, y_5, z_5)$ is the origin after all.
\end{proof}

\begin{proposition}
The limit set of a sphairahedron of the type (a) and of the type (b) is  the union of a plane and $\infty$. 
\end{proposition}

\begin{proof}
The 4 vertices $P_{125}, P_{235}, P_{345}, P_{145}$ are on the $xy$-plane and the limit set is the union of the $xy$-plane and $\infty$.
\end{proof}

In this pyramid  case, the group $G$ is $3$-Kleinian after all.  The coloring pattern of the limit set is generated by reflections on the plane.  See ****.  Here is four lines and one circle, ..... %あとで考えます。
