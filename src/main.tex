...
%Here we introduce another technical lemma.
%
%\begin{lemma}\label{tangencyOfThreeSphere}
%Let each $O_1, O_2, O_3$ be a sphere or a closed plane in $\subset S^3$.  Assume that $O_1\cap O_2\cap O_3$ is not empty.  For $(i,j)=(1,2), (1,3), (2,3)$, $e_{ij}=Let O_i\cap O_j$ is a circle or a line with $\infty$, and let \theta_{ij}$ be the dihedral angle of $O_i$ and $O_j$, where $0<\theta_{ij} \le \frac{\pi}{2}$.
%If $\theta_{12}+\theta_{13}+\theta_{23}=\pi$ then the intersection $O_1\cap O_2\cap O_3$ never consists of two points.
%\end{lemma}

%\begin{proof}
%Applying a Mobius transformation, we may assume that one of points in $O_1\cap O_2\cap O_3$ is the infinity $\infty$. Then $O_i$ is a plane with $\infty$ and $e_{ij}$ is a line with $\infty$.  If $O_1\cap O_2\cap O_3$ consists of two points, then $e_{ij}$'s intersect at a finite point transversally.  That is, three planes $O_1, O_2, O_3$ intersect transversally at a point $P$.  Consider a sphere $S$ with the center $P$ with a certain radius.  Then $S\cap O_i$ are an orthodromes and they establish a spherial triangle with inner angles $\theta_{12}, \theta_{13}, theta_{23}$ on $S$.  This contradicts $\theta_{12}+\theta_{13}+\theta_{23}=\pi$.
%\end{proof}
...
\begin{corollary}
\begin{align*}
 r_2 &= \dfrac{z_4z_6 + 6}{6} \\
 r_4 &= \dfrac{z_4(z_4-z_6) + 6}{6} \tag{**}\\
 r_6 &= \dfrac{z_6(z_6-z_4) + 6}{6}.
\end{align*}
\end{corollary}

We know that if $z_4$ and $z_6$ are given, we get the value of $r_2$, $r_4$, and $r_6$.  Here we have some conditions to establish a cube.  One is $r_2$, $r_4$, and $r_6$ are positive.  Secondly, $O_2\cap O_5=\O$, $O_4\cap O_3=\O$, and $O_6\cap O_1=\O$.  Thus, we obtain:
\[
0<r_2<\frac{3}{2},\ 0<r_6<\frac{3}{2},\ 0<r_6<\frac{3}{2}
\]
