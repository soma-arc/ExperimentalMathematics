...
rational.
However, when the arms overlap with rational angles.
It may be a discrete group of the group.
...
patterns.
We color the tiles according to the number of inversions.
We use the HSV color model to determine their color.
The color varies in order of red, yellow, green, and blue.
So, there are many sphairahedra on blue tiles.
The point is a parabolic fixed point, and the convergence of tile is
late.

When the sphairahedron at the outside of the parameter space,
the fractal bridge emerge.
The bridge passes through the prism sphairahedron.
The bridge seems to be discrete by n/pi.
So, there are parameters on our side of the parameter space.
We don't know that is point, curve, or regions.

In this case, we solve the mathematical problems obtained from rendered
images.

The outside of the parameter space, it makes a hole at the wall of the
prism. From the hole, the arms of the fractals are stretched, and 
There is a bridge.
Different edge it makes a hole by a different sphere.
It is not discrete, but when the arms are crossing at a rational angle, it becomes
discrete.
