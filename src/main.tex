...
\begin{definition}
 Let $P$ be an ideal sphairahedron and let $X$ be one of the vertex on $P$.
 Remark that, The edges are mutually tangent at the vertex $X$.
 Open sphere $B_x$ is maximum vertex ball about vertex X when the sphere
 ...
 \begin{description}
  ...
  \item[2)] $B_X \cap P \neq \phi$ and $B_x$ don't contains a vertex of
             $P$
  \item[3)] $B_X$ is a sphere satisfies 1) and 2), and it is
...
\begin{proof}
 ...
 \begin{description}
  ...
  \item[b)] For $F(P)=P'$, neighborhood of $F(x)$ is infinitely elongated
 ...
 vertex of $F(P)$ other than $x'$.
 
 It is given by $B'x' = \{(x, y, z) | z < z_0\}$ when
 $z_0 := \min\{ z | (x, y, z) \text{is an vertex of} F(P) \}$

$B'_{x'} \cap P'$ is right cylinder infinitely elongated for z-axis
...
Cube type sphairahedron type (a) is controlled by two parameter $z_b$
and $z_c$.
When the both parameters are zero, the limit set becomes plane (or
sphere) and like Figure \ref{fig:planes}\subref{fig:planeCube} there is
regular patterns. We will analyze the patterns.

When $z_b = z_c = 0$, the every center of the three spheres $C_A, C_B, C_C$
...
For example there are Figure \ref{fig:planes}\subref{fig:planeTetra} or
