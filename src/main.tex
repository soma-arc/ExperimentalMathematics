...
However, after Ahara and Araki publish 
, but stop the research of the sphairahedra.

After that Ryo Kageyama perform further experiment of
cube-type sphairahedron \cite{kageyama}, and he use lens space.

In 2017, breakthrough is came out. We develop an algorithm called
\textit{Iterated Inversion System (IIS)} and \textit{Sphere tracing}, a
kind of ray tracing technique.
Combining IIS and Sphere tracing are experiment



After Kazushi Ahara and Yoshiaki Araki published
We wrote \cite{bridges2018}.
In the paper, real-time rendering technique and 
discuss the fractal from Artistical perspective.
In this article, we discuss mathematical properties of
sphairahedra and its limit set.

Ryo kageyama and further experiment in
his master thesis\cite{kageyama}.

In this paper we focus pure mathematical viewpoint.

In 2003 after they publish a 4 page paper \cite{AharaAraki}.

In this paper, we consider sphairahedra and quasi-fuchsian fractals
mathematically.

We call them sphairahedra. sphairahedron is a coined word combining two
words sphaira- (a prefix that means ’spherical’) and -hedron (a suffix
comes from ’polyhedron.’)

The union of all the tiles
is mostly homeomorphic to a three-dimensional ball. Thus, this is called
a quasi-sphere. 
...
このように，プリントすることによって，レンダリングでみることのできない
角度からもフラクタルを観察することができる．
単色プリンタとカラープリンタによる3Dプリントを行った．
フラクタルとその対称性を肌で感じることができる．
We feel on the skin
fractal structures and its symmetry.
