...
Next, we prepare some lemmas to calculate the parameter space of sphairahedra. First, we recall a simple condition to be the equivalence of two sphairahedra.  This lemma is clear because it is well known that these transformations are M\"obius transformations. 

\begin{lemma}
Suppose that two sphairahedra $T_1, T_2$ satisfy that $T_2=\phi(T_1)$, where $\phi$ is a parallel translation or a rotation or a dilation or a plane-symmetry or a composite of these transformations.  Then $T_1, T_2$ are equivalent. 
\end{lemma}

Next, we consider a sphairahedron of infinite type without loss of generality.  In fact, $T$ is a spahairahedron, and  let $P$ be one of vertices and let $\varphi$ be a M\''obius transformation satisfying $\varphi(P)=\infty$.  $T'=\varphi(T)$ has the infinite vertex.  Thus we assume that one of the vertices is the infinite point.

Beacause of the ideality, edges with the infinite vertex are parallel straight lines.  Using rotations, we assume that the edges are parallel to $z$-axis.  It follows that the faces with the finite vertex are perpendicular with $xy$-plane.

Let $A$ be one of finite vertices next to the infinity vertex, that is, there exists an edge $e$ that connects $A$ and the infinity point.  After applying translations, we assume the followings:
\begin{align*}
A &= (0,0,0),\\ 
{S_1}  &\subset \{ 0 = y \} ,\\
{S_2}  &= \{ 0 = x\sin\theta_{12} - y\cos\theta_{12} \} ,\\
{S_3}  &= \{ (x-x_3)^2+(y-y_3)^2+(z-z_3)^2 = r_3^2 \}, 
\end{align*}
where $S_i$'s are faces, $theta_{12}$ is the dihedral angle of $S_1$ and $S_2$,  $(x_3, y_3, z_3)$ is the center of the face $S_3$, and $r_3$ is the radius of the face $S_3$
...
\bibitem[Soma 2020]{sphairahedron_net}
        Soma, 
