...
Let $T$ be a sphairahedron.  Let $\overline{D_i}$ ($i = 1,2,\ldots, p$) be the closed ball of $T$ and $S_i$ be the boundary sphere of $\overline{D_i}$.  Suppose that a map $I(S_i):S^3 \to S^3$ is the inversion map of $S_i$.  $I(S_i)$ is an orientation-reversing Moebius map on $S^3$ and let $G$ be a group generated by all $I(S_i)$'s.  That is,
\[
 G := \langle I(S_1), I(S_2), \ldots ,I(S_p)  \rangle < \text{Mob}(S^3)
\]
...
3-Kleinian group if $\Lambda(G)$ is a round sphere. 
A Kleinian group $G$ is called a
